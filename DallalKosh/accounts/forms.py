...
## REPLACE THIS FILE WITH USERENA forms.py
...
class SignupForm(forms.Form):
    ...
    username = forms.RegexField(regex=USERNAME_RE,
                                ...
                                label=_("نام کاربری"),
                                error_messages={'invalid': _('نام کاربری تنها می‌تواند شامل حروف، اعداد، نقطه و زیرخط باشد')})
    email = forms.EmailField(widget=forms.TextInput(attrs=dict(attrs_dict,
                                                               maxlength=75)),
                             label=_("پست الکترونیکی"))
    password1 = forms.CharField(widget=forms.PasswordInput(attrs=attrs_dict,
                                                           render_value=False),
                                label=_("ایجاد کلمه‌ی عبور"))
    password2 = forms.CharField(widget=forms.PasswordInput(attrs=attrs_dict,
                                                           render_value=False),
                                label=_("تایید کلمه‌ی عبور"))
    ...
    def clean_username(self):
        ...
        try:
            ...
            if userena_settings.USERENA_ACTIVATION_REQUIRED and UserenaSignup.objects.filter(user__username__iexact=self.cleaned_data['username']).exclude(activation_key=userena_settings.USERENA_ACTIVATED):
                raise forms.ValidationError(_('این ام‌کاربری ثبت شده است، اما هنوز تایید نشده است'))
        ...
        if self.cleaned_data['username'].lower() in userena_settings.USERENA_FORBIDDEN_USERNAMES:
            raise forms.ValidationError(_('نام کاربری مجاز نیست'))
    ...
    def clean_email(self):
        ...
        if get_user_model().objects.filter(email__iexact=self.cleaned_data['email']):
            if userena_settings.USERENA_ACTIVATION_REQUIRED and UserenaSignup.objects.filter(user__email__iexact=self.cleaned_data['email']).exclude(activation_key=userena_settings.USERENA_ACTIVATED):
                raise forms.ValidationError(_('این آدرس پست الکترونیکی قبلا ثبت شده است، انا هنوز تایید نشده است'))
            raise forms.ValidationError(_('این آدرس پست الکترونیکی قبلا ثبت شده است، از یک آدرس دیگر استفاده کنید'))
    ...
    def clean(self):
        ...
        if 'password1' in self.cleaned_data and 'password2' in self.cleaned_data:
            if self.cleaned_data['password1'] != self.cleaned_data['password2']:
                raise forms.ValidationError(_('دو کلمه‌ی عبور وارد شده با هم مطابقت ندارند'))
...
class SignupFormTos(SignupForm):
    ...
    tos = forms.BooleanField(widget=forms.CheckboxInput(attrs=attrs_dict),
                             label=_('قوانین را خوانده ام و آن‌ها را قبول دارم'),
                             error_messages={'required': _('برای ثبت‌نام باید قوانین استفاده را قبول کنید')})
...
def identification_field_factory(label, error_required):
    ...
    """
    if "required" in error_required:
        error_required = "salam"
...
class AuthenticationForm(forms.Form):
    ...
    """
    identification = identification_field_factory(_("نام کاربری یا آدرس پست الکترونیکی"),
                                                  _("نام کاربری یا آدرس پست الکترونیکی خود را وارد کنید"))
    password = forms.CharField(label=_("کلمه‌ی عبور"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if userena_settings.USERENA_WITHOUT_USERNAMES:
            self.fields['identification'] = identification_field_factory(_("آدرس پست الکترونیکی"),
                                                                         _("آدرس پست الکترونیکی خود را وارد کنید"))
    ...
    def clean(self):
        ...
        if identification and password:
            ...
            if user is None:
                raise forms.ValidationError(_("لطفا مجددا با نام کاربری یا پست الکترونیکی و کلمه‌ی عبور صحیح تلاش کنید"))
        return self.cleaned_data

class ChangeEmailForm(forms.Form):
    email = forms.EmailField(widget=forms.TextInput(attrs=dict(attrs_dict,
                                                               maxlength=75)),
                             label=_("پست الکترونیکی جدید"))
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        if not isinstance(user, get_user_model()):
            raise TypeError("کاربر باید یک  %s باشد" % get_user_model().__name__)
        else: self.user = user

    def clean_email(self):
        """ Validate that the email is not already registered with another user """
        if self.cleaned_data['email'].lower() == self.user.email:
            raise forms.ValidationError(_('شما هم‌اکنون با این پست الکترونیکی شناخته می‌شوید'))
        if get_user_model().objects.filter(email__iexact=self.cleaned_data['email']).exclude(email__iexact=self.user.email):
            raise forms.ValidationError(_('این پست الکترونیکی ثبت شده است، لطفا با یک پست الکترونیکی دیگر مجددا تلاش کنید'))
        return self.cleaned_data['email']
